...
from .serialization import dump_dill, dump_pickle, dill_protocol, pickle_protocol
from .error import SerializeError, PrintError, InitTypeError, Testing
...
class _MyInt(int):
    ...
    def __repr__(self):
        return f'{self.__class__.__name__} (Logger.{self.__str__})'


class LogLevel(_MyInt):
    pass


class SerializeType(_MyInt):
    pass
...
class Logger:
    ...
    level: LogLevel = field(default_factory=lambda: Logger.default_level)
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.serialize, SerializeType):
            raise InitTypeError(f"Parameter 'serialize' must be an instance of 'Logger.SerializeType', "
                                f"not {type(self.serialize)}")

        if not (isinstance(self.raise_error, tuple) and
                all([issubclass(elem, BaseException) for elem in self.raise_error])):
            raise InitTypeError("Parameter 'raise_error' must be an instance of 'tuple of BaseException'")

        if not (isinstance(self.ignore_error, tuple) and
                all([issubclass(elem, BaseException) for elem in self.ignore_error])):
            raise InitTypeError("Parameter 'ignore_error' must be an instance of 'tuple of BaseException'")
        ...
        if isinstance(self.out_file, TextIOBase):
            if not self.out_file.writable():
                raise InitTypeError("Parameter 'out_file' must be a writable")
...
return: {result}''')
    ...
    def __get__(self, instance, owner):
        ...
        p.__setattr__('_logger', self._logger)
        return p
...
class _AsyncLogger(_Logger):
    def __init__(self, *args, **kw):
        ...
        warn(Testing('Logger with async functions has known problems (sometimes variables are not shown).'),
             stacklevel=3)
...
def method_logger(parent: Logger, class_new, class_):
    for attr_name in dir(class_):
        ...
        if attr_name[:2] != '__' and callable(attr) and not getattr(attr, '_logger', False) and not isclass(attr):
